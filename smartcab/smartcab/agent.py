...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=False):
        ...
        if self.learning:
            self.trial_num += 1
            # reduce alpha.
            self.alpha = .999 ** self.trial_num
            self.epsilon = 1-math.exp(-8*math.exp(-self.trial_num/400.0))
        else:
            self.alpha = 0
            self.epsilon = 0
    ...
    def build_state(self):
        ...
        return inputs['left'], inputs['right'], inputs['oncoming'], inputs['light'], waypoint
    ...
    def get_maxQ(self, state):
        ...
        sorted_reward_actions = sorted([(reward, action) for action, reward in self.Q[state].items()], reverse=True)

        # Filter to keep the equal max
        max_reward_actions = filter(lambda i: i[1] == sorted_reward_actions[0][1], sorted_reward_actions)

        # Return random choice of max, usually a 1 element list though
        return random.choice(max_reward_actions)[1]
    ...
    def learn(self, state, action, reward):
        ...
        if self.learning:
            ...
            self.Q[state][action] = (reward * self.alpha) + (previous * (1-self.alpha))
...
def run():
    ...
    sim.run(n_test=100, tolerance=0.003)
